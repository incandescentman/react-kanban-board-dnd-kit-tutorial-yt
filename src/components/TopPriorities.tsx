...
export default function TopPriorities({ board, onSelectTask, onImportPinnedToBoard, preferredBoardTitle }: Props) {
  ...
  const getIconForPriority = (line: string, i: number) => {
    ...
    if (/\bsrs\b|spaced review|review system/.test(l)) return <IconBook2 size={24} stroke={2.25} className="shrink-0 mt-1 text-blue-700" aria-hidden="true" />;
    if (/food|urge/.test(l)) return <IconFlame size={24} stroke={2.25} className="shrink-0 mt-1 text-blue-700" aria-hidden="true" />;
    if (/185|pound|weight/.test(l)) return <IconScale size={22} stroke={2.25} className="shrink-0 mt-1 text-blue-700" aria-hidden="true" />;
    if (/substack|launch|publish|invite/.test(l)) return <IconRocket size={24} stroke={2.25} className="shrink-0 mt-1 text-blue-700" aria-hidden="true" />;
